...
- [ ] T002 Record the FR-031 pre-fix baseline: follow the procedure in `specs/109-redis-session-store-resilience/quickstart.md` to run the new regression specs against a worktree pinned at `caa1a0d33`, and capture the observed failure of each of FR-028/029/030 into the quickstart results table. This task is completed *after* the specs exist (T014, T021, T036) — it is listed here because its output belongs to Setup evidence, and is re-entered at T044
...
- [ ] T016 [US2] Add `SessionStoreUnavailableException extends BaseException` in `src/common/exceptions/session-store-unavailable.exception.ts`, carrying `AlkemioErrorStatus.SESSION_STORE_UNAVAILABLE` and setting `extensions.http.status = 503` so Apollo Server 4 overrides the wire status — without it Apollo emits HTTP 200 with an error envelope (contract G5). Export it from `src/common/exceptions/index.ts`. Depends on T014
...
- [ ] T037 Run the repo's full local exit gates in one uninterrupted run: `npm run lint`, the production build, and the full vitest suite. All three must pass clean before the PR opens
...
- T003 and T004 together; T007 and T008 together
