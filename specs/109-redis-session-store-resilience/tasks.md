...
- [ ] T042 **OUTSTANDING** — execute the live outage verification in `quickstart.md` §3–§5. Not run during implementation: the dev `alkemio_dev_redis` container is shared by ~20 running services, so stopping it for the required 3+ minutes is an outage of a working environment rather than a scoped test. Recorded as **NOT RUN** in the quickstart results table rather than assumed passed. Run against a disposable stack before the PR leaves draft
- [ ] T043 **OUTSTANDING** — re-run `108-redis-outage-resilience` SC-009 (SC-011), for the same reason as T042. Record it honestly — if it does not pass, say so rather than rewording it, which is the discipline #6331 set
