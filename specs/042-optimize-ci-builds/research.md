...
**Decision**: ~~Merge `ci-tests.yml` and `trigger-sonarqube.yml`~~ **DONE** — `ci-tests.yml` now contains both `test` and `sonarqube` jobs (`sonarqube` depends on `test` via `needs: test`). `trigger-sonarqube.yml` has been deleted.

**Rationale**: Previously, tests ran twice per PR — once without coverage in `ci-tests.yml` and once with coverage in `trigger-sonarqube.yml`. The merge eliminated this duplication: tests run once (with coverage) and the coverage artifact is shared with the SonarQube job.
...
2. `trigger-sonarqube.yml` — DONE: merged into ci-tests.yml and deleted
...
1. `build-release-docker-hub.yml` — Docker builds require Linux (sole remaining workflow after consolidation)
...
**Decision**: ~~Delete legacy and rename `-new` variant~~ **DONE** — `build-release-docker-hub.yml` is now the sole Docker release workflow. The legacy workflow was deleted and `build-release-docker-hub-new.yml` was renamed to `build-release-docker-hub.yml`.

**Rationale**: Both workflows previously triggered on `release: [published]` events, causing duplicate Docker builds. The newer workflow used `docker/metadata-action@v5` for tag generation, which was cleaner and more maintainable than the manual tag parsing in the legacy workflow. The consolidation eliminated duplicate builds and maintenance overhead.
