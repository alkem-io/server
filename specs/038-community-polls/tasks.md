...
- [X] T008 [P] Create `src/domain/collaboration/poll/poll.settings.interface.ts` — `@ObjectType('PollSettings') abstract class IPollSettings` with `@Field` decorators for `minResponses: Int!`, `maxResponses: Int!`, `resultsVisibility: PollResultsVisibility!`, `resultsDetail: PollResultsDetail!` (all immutable, per data-model.md)
...
- [X] T028 [P] [US1] Create `CreatePollInput` input DTO in `src/domain/collaboration/poll/dto/poll.dto.create.ts`: required `title: String!` (max 512 chars, `@MaxLength(MID_TEXT_LENGTH)`), optional `settings?: PollSettingsInput`, required `options: [String!]!` (each max 512 chars, `@ArrayMinSize(2)`)
