...
**Checkpoint**: Column + GraphQL field + migration in place. The field is now readable on every existing query path that already returns `UserSettings`. All pre-existing rows now carry `1`.
...
1. Setup + Foundational → field is queryable; all rows carry `1`.
