...
| -------- | ---- | --------- | ------- | -------- | ------- | ----- |
| `designVersion` | `number` | `designVersion` | `int` | `NOT NULL` | `2` | UI design generation for the user. `1` = previous design, `2` = current default ("new"), `3+` reserved for future designs. |
...
@Field(() => Int, {
  nullable: false,
  description: 'The design version this User has selected (1 = previous design, 2 = current default, 3+ reserved for future designs).',
})
designVersion!: number;
```
...
| Default = 2 | FR-002, FR-003 | DB column default + explicit seeding in `UserService.getDefaultUserSettings()`. Belt-and-braces — either layer alone would be sufficient. |
...
ALTER TABLE "user_settings"
  ADD "designVersion" int NOT NULL DEFAULT 2;
```
...
- The `NOT NULL DEFAULT 2` form atomically backfills every pre-existing row to `2` as part of the `ADD COLUMN` (FR-009 satisfied by the DDL alone — no separate `UPDATE` needed).
...
`UserService.getDefaultUserSettings()` (line 276 of `src/domain/community/user/user.service.ts`) returns the `CreateUserSettingsInput` used for every newly registered user. Add `designVersion: 2` to the returned object, alongside the existing `homeSpace`, `privacy`, `communication`, and `notification` defaults. This explicit seeding makes FR-003 ("every new user gets 2") visible at the registration call site, independent of the DB default.

`UserSettingsService.createUserSettings()` (line 27 of `src/domain/community/user-settings/user.settings.service.ts`) currently spreads four fields from the input. Add `designVersion: settingsData.designVersion ?? 2` to the spread, so callers that pass an explicit value win and callers that omit it still get 2.
...
(Use `!== undefined`, not truthiness, so that `0` and negative integers are accepted per FR-004.)
