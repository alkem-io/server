...
- The two new RPC handlers live in a new lightweight controller `MatrixRoomCheckController` under `src/services/adapters/communication-adapter/` (sibling of the existing event service), using `@MessagePattern` with `Transport.RMQ` — the same pattern already used by `WhiteboardIntegrationController` and `CollaborativeDocumentIntegrationController`.
...
**Primary Dependencies**: NestJS 10 (`@nestjs/event-emitter`, `@nestjs/microservices` for `@MessagePattern` + `Transport.RMQ`), `@golevelup/nestjs-rabbitmq` (existing, used by the adapter event service), TypeORM 0.3 (custom fork `pkg.pr.new/antst/typeorm`), Apollo Server 4 / GraphQL 16 (the `conversationCreated` subscription already exists — not modified). `@alkemio/matrix-adapter-lib` is bumped to the PR-channel release at `https://pkg.pr.new/@alkemio/matrix-adapter-lib@53` (matrix-adapter-go PR #53), which exposes `MatrixAdapterEventType.COMMUNICATION_ROOM_CHECK` / `COMMUNICATION_ROOM_INFO` constants and the `CheckRoomRequest` / `CheckRoomResponse` / `GetRoomInfoRequest` / `GetRoomInfoResponse` / `RoomInfoMember` interfaces. We use the lib interfaces directly as `@MessagePattern` payload types — same pattern as the existing `CommunicationAdapterEventService.onMessageReceived` (typed with `MessageReceivedPayload` from the lib). No server-local wrapper DTOs. Pre-merge, the pkg.pr.new URL gets swapped for a released semver tag once the adapter cuts a real release.
...
└── services/
    └── adapters/
        └── communication-adapter/
            ...
            ├── matrix.room.check.controller.ts   # NEW: @MessagePattern handlers for room.check and room.info; payload types imported from @alkemio/matrix-adapter-lib (same pattern as onMessageReceived)
...
**Structure Decision**: Standard single-project layout per CLAUDE.md. The controller follows the precedent set by `WhiteboardIntegrationController` and `CollaborativeDocumentIntegrationController` (NestJS `@MessagePattern` + `Transport.RMQ`). The displayName helper lives in `src/domain/actor/` since it operates on Actor-shaped data (User and VC are both Actors via NameableEntity).
...
- **R-001 RPC handler shape**: NestJS `@MessagePattern(routingKey, Transport.RMQ)` on a new lightweight controller — same pattern as `WhiteboardIntegrationController:39`. Reply value is the method return; NestJS RabbitMQ transport handles correlation-id wiring transparently.
- **R-002 Routing key constants and DTO interfaces**: `@alkemio/matrix-adapter-lib` is bumped to the pkg.pr.new PR-channel build from matrix-adapter-go PR #53, which exposes `MatrixAdapterEventType.COMMUNICATION_ROOM_CHECK`, `MatrixAdapterEventType.COMMUNICATION_ROOM_INFO`, and the five wire interfaces (`CheckRoomRequest`, `CheckRoomResponse`, `GetRoomInfoRequest`, `GetRoomInfoResponse`, `RoomInfoMember`). The server uses these interfaces directly as `@MessagePattern` payload types — matching the existing precedent of `onMessageReceived(payload: MessageReceivedPayload)` in `CommunicationAdapterEventService`. No server-local wrapper DTOs and no `class-validator` decorators (the codebase's `dto/` folder under communication-adapter has zero validator decorators — the existing convention is to validate at the domain boundary, which the plan does in T027). Pre-merge follow-up: swap the pkg.pr.new URL for a released semver tag once the adapter release ships.
- **R-003 `CheckResult` discriminated union**: `{ kind: 'accepted'; alkemioRoomId: string } | { kind: 'rejected'; reason: string }`. The controller translates the union to the wire-level `{ allow: boolean, ... }` envelope. One-line conversion at the controller boundary.
