...
- **FR-002**: The platform MUST validate the request shape: identifiers are non-empty and well-formed; the creator is not present in the member list; the member list has no duplicates; the member count matches the type (exactly one for direct, one-or-more for group).
