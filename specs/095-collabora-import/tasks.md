...
- [ ] T001 Confirm dev services are running: `pnpm run start:services` (Postgres 17.5, RabbitMQ, Redis, Kratos/Oathkeeper, file-service-go)
- [ ] T002 Apply migrations: `pnpm run migration:run` (this feature adds zero migrations; this guards against drift)
- [ ] T003 [P] Snapshot the current GraphQL schema baseline before edits: `pnpm run schema:print && pnpm run schema:sort && cp schema.graphql tmp/prev.schema.graphql` — establishes the baseline for SC-006 schema-diff verification
...
- [ ] T005 Verify `CollaboraDocumentService` is already injected into `CalloutFramingService` constructor at `src/domain/collaboration/callout-framing/callout.framing.service.ts` (it must be — `createCalloutFraming` already calls `this.collaboraDocumentService.createCollaboraDocument(...)`). If not, add it to the constructor and the module's `providers`/`exports`.
- [ ] T006 Verify `streamToBuffer` is importable from `@common/utils/file.util` and `storage.file.stream_timeout_ms` config key resolves (used by the existing `importCollaboraDocument` resolver at `src/domain/collaboration/callout/callout.resolver.mutations.ts:479-483`). The new resolver branch will reuse both.
- [ ] T007 Verify `FileServiceAdapterException` and `StorageServiceUnavailableException` are exported from `src/services/adapters/file-service-adapter/file.service.adapter.exception.ts` and that the `mapHttpStatusToAlkemioStatus` helper covers 415/422/413/503 (per research § R2). No code changes; just confirm.
...
- [ ] T008 [P] [US1] Run the existing blank-path unit tests as a pre-change baseline: `pnpm test -- src/domain/collaboration/callout-framing/callout.framing.service.spec.ts src/domain/collaboration/collabora-document/collabora.document.service.spec.ts src/domain/collaboration/callouts-set/callouts.set.resolver.mutations.spec.ts`. Capture pass/fail counts in commit message; this becomes the SC-001 reference.
- [ ] T009 [P] [US1] Run the existing blank-path integration tests as a pre-change baseline: `pnpm test -- test/integration/` filtering to any existing collabora / callout-creation specs. Capture baseline.
- [ ] T010 [P] [US1] After all US2/US3/US4 implementation tasks land, re-run T008 and T009 — assert zero regressions. This is the SC-001 verification.
...
- [ ] T011 [P] [US2] Add unit test in `src/domain/collaboration/callout-framing/callout.framing.service.spec.ts` for `createCalloutFraming` upload branch: when `calloutFramingData.collaboraDocument._uploadedFile` is present, asserts `collaboraDocumentService.importCollaboraDocument` is called with the buffered file and the input `displayName` (or undefined when absent), and `createCollaboraDocument` is NOT called. Use the existing mock pattern in this file.
- [ ] T012 [P] [US2] Add unit test in `src/domain/collaboration/callout-framing/callout.framing.service.spec.ts` for `createCalloutFraming` blank branch unchanged: when `_uploadedFile` is absent, asserts `createCollaboraDocument` is called and `importCollaboraDocument` is not. Validates input requires `displayName` and `documentType` (throws `ValidationException` if either is absent).
- [ ] T013 [P] [US2] Add unit test in `src/domain/collaboration/callouts-set/callouts.set.resolver.mutations.spec.ts` for the resolver: when `file` arg is supplied, the resolver buffers with `streamToBuffer(createReadStream(), streamTimeoutMs)` and passes the buffered `{ buffer, filename, mimetype }` through `calloutData.framing.collaboraDocument._uploadedFile` (or whatever transient-field name the implementation uses) before invoking `calloutsSetService.createCalloutOnCalloutsSet`.
- [ ] T014 [P] [US2] Add unit test asserting authorization is checked exactly once at the resolver entry (FR-007), with the same `AuthorizationPrivilege.CREATE_CALLOUT` privilege used today on the blank path.
- [ ] T015 [P] [US2] Add integration test `test/integration/collabora-document-framing-import.it-spec.ts` for the happy path: authenticated caller posts a multipart `createCalloutOnCalloutsSet` with a real DOCX fixture; assert one new callout exists, one new framing Collabora document exists, one new storage object exists; assert `documentType = WORDPROCESSING`; assert `profile.displayName` defaults from filename when input `displayName` is empty (FR-012).
- [ ] T016 [US2] Run T011–T015 and confirm all FAIL with the expected "method not implemented / arg not accepted / field not present" errors before any implementation lands.

### Implementation for User Story 2

- [ ] T017 [US2] Modify `src/domain/collaboration/collabora-document/dto/collabora.document.dto.create.ts`: relax `displayName` from `@IsNotEmpty()` to `@IsOptional() @IsString()`; relax `documentType` to `@IsOptional() @IsEnum(CollaboraDocumentType)`; update `@Field()` decorators on both to add `nullable: true` and update descriptions to note "Required for blank-create. On the upload path, displayName defaults from the uploaded filename and documentType is derived from sniffed MIME." Add a non-GraphQL transient field `_uploadedFile?: { buffer: Buffer; filename: string; mimetype: string }` (no `@Field()` decorator — mirrors the `mediaGallery` pattern at `callout.framing.dto.create.ts:74-79`) to carry the buffered file from resolver to service.
- [ ] T018 [US2] Modify `src/domain/collaboration/callout-framing/callout.framing.service.ts` `createCalloutFraming` at lines 176–190: replace the unconditional `createCollaboraDocument` call with a branch — if `calloutFramingData.collaboraDocument?._uploadedFile` is present, call `this.collaboraDocumentService.importCollaboraDocument(_uploadedFile, calloutFramingData.collaboraDocument.displayName, storageAggregator, userID)` and assign the result to `calloutFraming.collaboraDocument`; else call the existing `createCollaboraDocument(calloutFramingData.collaboraDocument, storageAggregator, userID)`. Add a service-layer validation: if no `_uploadedFile` and (no `displayName` OR no `documentType`), throw `ValidationException('Callout Framing of type COLLABORA_DOCUMENT requires displayName and documentType when no file is supplied.', LogContext.COLLABORATION)`.
- [ ] T019 [US2] Modify `src/domain/collaboration/callouts-set/callouts.set.resolver.mutations.ts` `createCalloutOnCalloutsSet`: add a new optional second arg `@Args({ name: 'file', type: () => GraphQLUpload, nullable: true }) fileUpload?: FileUpload`. Imports: `import { FileUpload, GraphQLUpload } from 'graphql-upload'`, `import { streamToBuffer } from '@common/utils/file.util'`, and inject `ConfigService`. When `fileUpload` is present, buffer it with `streamToBuffer(fileUpload.createReadStream(), streamTimeoutMs)` (resolve `streamTimeoutMs` from `configService.get<number>('storage.file.stream_timeout_ms')`), then attach `{ buffer, filename: fileUpload.filename, mimetype: fileUpload.mimetype }` to `calloutData.framing?.collaboraDocument._uploadedFile` before calling `calloutsSetService.createCalloutOnCalloutsSet`. If `fileUpload` is supplied but `calloutData.framing?.type !== COLLABORA_DOCUMENT` or `calloutData.framing.collaboraDocument` is absent, throw `ValidationException('file argument is only valid with framing.type = COLLABORA_DOCUMENT and framing.collaboraDocument supplied', LogContext.COLLABORATION)` BEFORE buffering.
- [ ] T020 [US2] Update the GraphQL description on the `createCalloutOnCalloutsSet` mutation in the same resolver file to reflect the new optional `file` arg per `contracts/schema.delta.graphql`.
- [ ] T021 [US2] Add Winston log lines using `LogContext.COLLABORATION` at: (a) resolver entry when `fileUpload` is present (debug — "creating callout with uploaded framing document, filename={}, size={}"); (b) framing service branch decision (debug — "framing.collaboraDocument resolution: {blank|upload}"); (c) on `StorageServiceUnavailableException` propagation (warn — surfaces upstream unavailable per FR-009 / clarification Q6 fail-fast). No log lines on the happy path other than what already fires; per Constitution Principle 5, instrument only what's actively consumed.
- [ ] T022 [US2] Run T011–T015 and confirm all PASS.
- [ ] T023 [US2] Regenerate the schema and verify the diff matches `contracts/schema.delta.graphql`: `pnpm run schema:print && pnpm run schema:sort && pnpm run schema:diff`. Inspect `change-report.json` — assert: 3 additive entries (`CreateCollaboraDocumentData.displayName` required→optional, `CreateCollaboraDocumentData.documentType` required→optional, `Mutation.createCalloutOnCalloutsSet.file: Upload` added optional); 0 BREAKING entries.
