...
The feature is done when, and only when, the following three-phase end-to-end verification passes for a target Space `S` on a freshly-migrated environment:
...
3. **Then** `SPACE_FLAG_OFFICE_DOCUMENTS` MUST report as **disabled**, where "disabled" is defined as `limit = 0` (the canonical gating signal — a capability is gated off iff `limit = 0`). The `enabled` field may be `true` on the Space license default and `false` on the Collaboration license default, matching the existing `SPACE_FLAG_MEMO_MULTI_USER` precedent; the gating decision is driven by `limit`, not by `enabled`, at this stage. The entitlement entry may be present in the set in its disabled default state — what is NOT acceptable is for `limit ≥ 1`.
...
9. **Then** `SPACE_FLAG_OFFICE_DOCUMENTS` MUST report as **disabled** (`limit = 0`) on `S`'s Space license and on every Collaboration in `S`'s hierarchy, with no residual `limit ≥ 1` state.
...
- **FR-007**: Newly created Spaces / Collaborations MUST automatically include the new entitlement in their entitlement set, defaulted to disabled (`limit = 0`; the `enabled` field follows the existing precedent — `true` on Space-level defaults, `false` on Collaboration-level defaults), without any manual intervention by operators.
- **FR-008**: Existing Spaces / Collaborations gain the new entitlement (defaulted to disabled, i.e., `limit = 0`) lazily the next time the license policy is applied to them (e.g., on a normal license-policy recalculation, not triggered by this feature). No backfill is performed by the migration; no Space holds the new credential immediately after the migration runs.
