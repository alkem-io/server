...
- [x] T023 [P] [US3] Create `AssignConversationMemberInput` DTO in `src/domain/communication/conversation/dto/conversation.dto.add-member.ts` — fields: `conversationID: UUID`, `memberID: UUID`
...
- [x] T050 Change mutation return types from `Conversation`/`Conversation?` to `Boolean!` for `assignConversationMember`, `removeConversationMember`, `leaveConversation`, `updateConversation`
