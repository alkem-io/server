...
- **FR-018**: Authorization privileges for mutations: `CREATE` on the platform Messaging for `createConversation`, `CONTRIBUTE` on the conversation for `addConversationMember`, `removeConversationMember`, and `updateConversation`, `READ` on the conversation for `leaveConversation`, `DELETE` on the conversation for `deleteConversation`.
