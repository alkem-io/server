...
- Clarify session (2026-06-13): (1) "members only" = **accepted members** (existing member role, includes admins/leads; excludes pending/non-members) — FR-028c; (2) guaranteed presence is **"at least one of each"**, server adds only if a tab has none (no hard dedupe) — FR-020c; (3) existing L0 spaces backfilled by a **one-time idempotent DB migration** — FR-020d; (4) spaces callout must **preserve existing subspace sorting + pinning** and migrate to identical behaviour — FR-025a (SC-017). Per an earlier user instruction, answers were integrated directly into requirements rather than into a Clarifications Q&A log.
- Stakeholder updates 2026-06-16 (during `/speckit-plan`): (a) the spaces callout derives subspace **ordering + pinning from the space** (space sort mode + per-subspace sortOrder/pinned); **no callout-level sort/pin control** in this version, may be added later (FR-025a). (b) **Reversed the position-lock**: the callout type is admin-creatable via the standard callout creation flow (FR-022), and instances — including the provisioned defaults — are **moved/reordered/deleted like any other callout**; only the actor kind stays immutable. Default L0 presence is now **"provision once, then free"**: templates + creation-time enforcement + one-time backfill seed the defaults, but the server does not re-add a deliberately deleted default (FR-020a, US5, SC-008/010/011 updated). (c) **Hide publisher** captured as a **parked open question / Future Consideration** (needs wider discussion: scope per-callout vs per-space vs platform, data-boundary vs UI-only, audit interaction) — not built in this version.
- Spec passes all quality items; ready for `/speckit-plan`.
