...
- **FR-025a**: System MUST, for a spaces callout, preserve the existing subspace **sorting and pinning** options that the platform currently supports for subspaces — the spaces callout MUST honour the same per-space sort order and pinned-subspace settings as the hard-coded subspaces display it replaces. The rollout MUST migrate existing spaces so their subspace ordering and pinning behave identically after the change (no reordering or loss of pins; consistent with SC-011).
