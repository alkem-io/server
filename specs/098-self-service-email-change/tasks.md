...
- [ ] `email_change_audit_outcome` Postgres enum exists with 097's 11 values (`committed`, `rolled_back`, `drift_detected`, `drift_resolved`, `drift_resolution_failed`, `security_signal_failed`, `new_address_notification_failed`, `global_admin_notification_failed`, `session_invalidation_failed`, `rejected_validation`, `rejected_conflict`).
...
- [ ] All three of 097's notification events exist and have publish helpers on `NotificationAdapter` (097 T013 + T015): `USER_EMAIL_CHANGE_SECURITY_SIGNAL` (OLD-address signal, helper `publishEmailChangeSecuritySignal`), `USER_EMAIL_CHANGE_NEW_ADDRESS_NOTIFICATION` (NEW-address acknowledgement, helper `publishEmailChangeNewAddressNotification`), `USER_EMAIL_CHANGE_GLOBAL_ADMIN_NOTIFICATION` (admin fan-out via the existing Global-Role-Change pattern, helper `publishEmailChangeGlobalAdminNotification`). 097's `applyAdminEmailChange` already publishes all three post-commit; 098's `confirm` method reuses the same publish chain.
...
- [ ] ~~S001~~ **INHERITED FROM 097** — `endpoints.client_web: string` is added to `AlkemioConfig` by 097 (tasks T000a/T000b) because FR-016c's NEW-address notification needs a login link. 098 reuses the existing key unchanged. This task ID is reserved; no work to do here.
- [ ] ~~S002~~ **INHERITED FROM 097** — config-loader wiring is done by 097 (T000b). No work to do here.
...
- [ ] S005 Add `publishEmailChangeConfirmation(payload)` helper on `NotificationAdapter` (alongside 097's three existing helpers: `publishEmailChangeSecuritySignal`, `publishEmailChangeNewAddressNotification`, `publishEmailChangeGlobalAdminNotification`); coordinate matching template addition in `@alkemio/notifications-lib` and bump dependency version (R8). This brings the total to FOUR email-change notification events across both specs.
- [ ] S006 [P] Create `UserEmailChangeState` enum (7 values: `INITIATED`, `CONFIRMED`, `COMMITTED`, `ROLLED_BACK`, `EXPIRED`, `SUPERSEDED`, `DRIFT_DETECTED`) in `src/domain/community/user-email-change/enums/user.email.change.state.ts` (FR-021-EXT)
...
- [ ] S024 [P] [US1] Integration spec `test/functional/integration/user-email-change-self.it-spec.ts` covering self-service end-to-end against real Postgres + mocked Kratos HTTP: Alice authenticates → initiates → MailSlurper assertion: ONE confirmation message at the NEW address (role tag = `self`, confirmation link present, no admin profile in message per FR-003b) → confirms session-lessly via root mutation → asserts `user.email` updated + Kratos identity trait updated (with `verifiable_addresses` verified per 097's FR-011) + `disableIdentitySessions` called. **Post-commit notification assertions (all three of 097's events must fire with `initiator_role: SELF`):** (a) ONE security-signal at the OLD address with role tag `self` and masked new email (097's FR-016 inherited); (b) ONE acknowledgement at the NEW address with role tag `self` and the FULL new email + login URL built from `endpoints.client_web` (097's FR-016c inherited); (c) ONE `USER_EMAIL_CHANGE_GLOBAL_ADMIN_NOTIFICATION` event published to the notifications-service inbox with `initiatorRole: SELF` and `triggerOutcome: COMMITTED` (097's FR-016d inherited — assert ONLY the publish; downstream per-recipient delivery is out of scope per 097's FR-016d). **End-of-flow mailbox count assertions (US1 Scenario 5)**: at the OLD address, MailSlurper shows EXACTLY ONE message (the FR-016 security signal). At the NEW address, MailSlurper shows EXACTLY TWO messages in chronological order — first the pre-commit FR-003 confirmation (containing the clickable token link), then the post-commit FR-016c acknowledgement (containing the full new email and the login link). For each of the three post-commit notifications, the per-event-fail path (mock the adapter to reject) MUST be covered separately and assert: commit still stands, matching `security_signal_failed` / `new_address_notification_failed` / `global_admin_notification_failed` audit entry is written, no rollback fires. Run the full happy-path twice with the Kratos mock identity shaped as (a) password-only and (b) oidc-only (FR-001a).
...
## Phase 6: Cross-flow coordination (Blocking for spec correctness)

**Purpose**: Implement the edge-case contract from spec.md §Edge Cases: when an admin invokes 097's synchronous `adminUserEmailChange` while a self-initiated pending change is in flight for the same subject, the pending row MUST transition to `SUPERSEDED` so the prior token can no longer be confirmed and `me.pendingEmailChange` no longer returns the stale row. This wiring lives in 098 (because 097 does not import the pending entity) and extends 097's service method additively.

- [ ] S047 [US1] Extend `UserEmailChangeService.applyAdminEmailChange` (owned by 097) in `src/domain/community/user-email-change/user.email.change.service.ts`: after the validation pipeline passes and BEFORE the Kratos write, look up any active `email_change_pending` row for the subject (state ∈ {`INITIATED`, `CONFIRMED`}); if present, transition it to `SUPERSEDED` (set `failure_reason: superseded_by_admin`, `updated_date: now()`) and append a `superseded` audit entry with `initiator_role: PLATFORM_ADMIN` and `failure_reason: superseded_by_admin` to mirror the transition. The supersession MUST happen in the same TypeORM transaction as the Alkemio side-write so it rolls back atomically on commit failure. (Spec.md §Edge Cases — cross-flow supersession)
- [ ] S048 [US1] Integration spec `test/functional/integration/user-email-change-cross-flow.it-spec.ts`: (a) Alice self-initiates a change to `target@…`; (b) Polly admin-invokes `adminUserEmailChange` for Alice with a different target email `admin-target@…`; (c) admin mutation commits successfully → assert Alkemio + Kratos both reflect `admin-target@…` AND Alice's prior `email_change_pending` row is in state `SUPERSEDED` AND an audit entry with outcome `superseded` and `initiator_role: PLATFORM_ADMIN` exists; (d) Alice attempts to confirm with her old token → asserts `EMAIL_CHANGE_TOKEN_INVALID`; (e) Alice queries `me.pendingEmailChange` → null.
...
- **Phase 6 (Cross-flow coordination)**: depends on Phase 3 (S018 creates pending rows that the admin path supersedes). Blocking for spec correctness — the edge-case contract from spec.md §Edge Cases ("admin synchronously supersedes in-flight self-initiated pending row") is only satisfied once this phase lands.
- **Phase 7 (Polish)**: depends on every user story phase intended for the release.
