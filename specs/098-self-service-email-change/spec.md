...
6. **Given** the user has completed the change, **When** the global-admin notifications pipeline is inspected, **Then** exactly one `USER_EMAIL_CHANGE_GLOBAL_ADMIN_NOTIFICATION` event has been published with `initiatorRole: SELF` and `triggerOutcome: COMMITTED`, fanned out by the existing notifications service to every platform admin via email + push + in-app — identical channel-fan-out behaviour to the Global-Role-Change notification (097's FR-016d). Rejected or rolled-back confirmations publish zero events.
