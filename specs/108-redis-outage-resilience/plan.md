...
| FR-015/FR-016, G1 · **T8** — the outage that emits no `error` at all is still reported | Unit: emit **only** `'reconnecting'` on the fake client; assert exactly 1 record and `connectionSignal.isDown === true`, then `'ready'` → 1 more record and `false`. `cache.store.factory.spec.ts` "reports a loss signalled only as `reconnecting`, with no error event". This is the ordinary outage (`retry_strategy` set ⇒ no `error` re-emit, research R6), so a suite asserting only on `'error'` would stay green while a real outage went silent. |
| FR-010a, G5 — the live signal is published, and tracks the cycle | Unit: `cache.store.factory.spec.ts` "publishes the connection signal for direct-client consumers"; `cache.connection.reporter.spec.ts` "tracks the connection state across an outage cycle". Consumer side: `task.service.spec.ts` "does not log per failed item claim while the connection is down" (passes an `itemKey`, so it reaches the failing `SADD`) and "still logs when there is no outage signal". |
| FR-017, G6 · **T10** — mutation records are gated on the live state | Unit: `cache.store.factory.spec.ts` "reports an invalidation that failed against a REACHABLE cache" (connection up ⇒ **1** record, per occurrence — the stale-entry case the transition record cannot cover) paired with "stays silent about mutations once the outage has been reported" (connection down ⇒ **0**). |
| FR-017 — failed reads are never reported | Unit: `cache.store.factory.spec.ts` "does not log per failed read". A failed read is indistinguishable from a cold cache, so logging it would be the flood, not the signal. |
| FR-019, G3 · **T11** — no secrets in any record | Unit: `cache.connection.reporter.spec.ts` "carries the error message and code, and nothing else from the error" and "handles a non-Error rejection without throwing"; "routes every record through the injected logger with a context" (G4). |
| FR-020, G6 — the direct-store surface fails soft, shape-preserving | Unit, `cache.store.factory.spec.ts`: "fails mget soft, preserving the positional shape callers index into" (→ array of `undefined` at the requested arity — `RoleSetCacheService` reads `store.mget` directly, research "Surface to wrap"), "fails keys soft, yielding an empty list rather than undefined" (→ `[]`), "fails ttl soft, reporting \"no expiry known\" rather than a bogus number" (→ `-1`), "turns a failing multi-key write into a no-op" (`mset` resolves). Task obligations: [tasks.md](./tasks.md) T007/T008. |
| Reporter contract T1–T7 in full | `cache.connection.reporter.spec.ts`, per [contracts/connection-state-reporter.md](./contracts/connection-state-reporter.md); task obligation [tasks.md](./tasks.md) T005. |
