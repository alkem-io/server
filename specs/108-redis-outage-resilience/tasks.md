...
- [X] T006 [US1] [US2] [US3] Implement `createRedisCacheStore(config, logger)` in `src/core/cache/cache.store.factory.ts` per [contracts/cache-store-factory.md](./contracts/cache-store-factory.md). Returns a `cache-manager` store-factory function. Inside it:
  ...
  2. construct a `CacheConnectionReporter` and attach `client.on('error', …)`, `client.on('reconnecting', params => reporter.reportError(params?.error))` and `client.on('ready', …)` **before returning**, with no `await` in between (G2 — the `'error'` line is what stops the process dying). The `reconnecting` listener is **not** optional and **not** redundant: `redis@3.1.2` re-emits a socket failure as `'error'` only when no `retry_strategy` is set (`index.js:341`), and step 1 sets one — so the ordinary outage (`docker stop redis`, ECONNREFUSED, peer close) arrives as `reconnecting` and never as `error`. Without it the common outage is silent and `isDown` never flips, breaking FR-010a and FR-015 – FR-019;
  3. expose a read-only `connectionSignal` on the returned store that prefers the client's authoritative `client.ready` flag and falls back to `reporter.isDown` only when the client does not expose one (FR-010a — a one-off non-connection `'error'` would otherwise latch the reporter's boolean down for the life of the process);
  4. return the store wrapped by the fail-soft proxy from T007.
  - *Acceptance*: FR-003, FR-010a, FR-011 – FR-014, FR-020, FR-021; G1, G2, G5, G8. Reporter contract: `reconnecting` listener per its consumer obligations; test obligation T8.

- [X] T007 [US1] Implement the fail-soft wrapper inside `src/core/cache/cache.store.factory.ts`: spread the original store (preserving `name` and `getClient` — G6, and `TaskService` depends on it), overriding `get`, `set`, `del`, `reset`, **`mget`, `mset`, `keys` and `ttl`**. `get` resolves `undefined` on any rejection; `set`/`del`/`reset`/`mset` resolve. Catch **broadly**, never by error code (FR-008). Race every call against a **1000 ms** ceiling (FR-009a), skipping the timer when the connection signal already reports down (the client rejects instantly there, so arming a timer per call would be pure overhead on a hot path for the whole outage). Forward `set`'s third argument to the underlying store **unmodified** — this preserves the legacy `{ ttl: seconds }` semantics and is the guarantee the whole design rests on (G7, research R4).
  - The multi-key and introspection methods are **not** covered by the spread alone, and `mget` is genuinely exercised: `RoleSetCacheService` (`role.set.service.cache.ts:57`) reads `cacheManager.store.mget` and calls it on every batched membership lookup, so leaving it unwrapped keeps that path's old behaviour — a rejection per request during an outage, and no ceiling at all against a connected-but-silent server. `mset`/`keys`/`ttl` have no consumer today and are wrapped defensively, so a future consumer inherits the protection instead of silently reintroducing the gap (FR-020). `mdel` is neither wrapped nor used — no consumer and not part of the store surface `cache-manager@5` drives here.
  - Each fallback must be **shape-preserving**, since callers consume the result positionally or structurally: `mget` → an array of `undefined` of the same arity as the request (a bare `undefined` would turn a cache miss into a `TypeError`), `keys` → `[]`, `ttl` → `-1` (node_redis' "no expiry known", the safest claim about a key we could not reach).
  - Failure logging is gated: mutations report a failure **only** while the connection signal reports up; reads never report. FR-017.
  - *Acceptance*: FR-005 – FR-009a, FR-017, FR-020; G3, G4, G6, G7. Depends on T006.
...
- [X] T008 [P] [US1] Write `src/core/cache/cache.store.factory.spec.ts` covering contract test obligations T1–T8, driving the factory with a substitute client/store that fails on demand — **not** through a consumer, since consumers already tolerate misses and such a test would pass against `develop` and prove nothing (FR-027). Assert: emitting `'error'` does not throw; rejecting `get` → `undefined`; rejecting `set`/`del` → resolve; a never-settling operation resolves at the 1 s ceiling (fake timers); `retry_strategy` returns a number for attempts 1…100, monotonic, capped at 5000, never an `Error`; `getClient` and `name` survive wrapping; construction with an unreachable host does not throw; `set` forwards its third argument unmodified.
  - Additionally assert the **`reconnecting`** path and the extra methods: emitting `'reconnecting'` with no preceding `'error'` still produces exactly one record and flips the signal (this is the *ordinary* outage — a suite that only emits `'error'` would pass while the real failure mode stayed silent); and each of `mget`/`keys`/`ttl` returns its shape-preserving fallback on rejection — `mget` an array of `undefined` matching the requested arity, `keys` `[]`, `ttl` `-1` — with `mset` resolving rather than rejecting.
  - *Acceptance*: FR-024 – FR-027, SC-007; reporter contract test obligations T8 – T11. Depends on T006, T007.
...
- [X] T009 [US1] Replace the inline cache configuration in `src/app.module.ts` (currently lines ~142–156, including the `import * as redisStore from 'cache-manager-redis-store'` at line 107) with a call to `createRedisCacheStore`. Inject `WINSTON_MODULE_NEST_PROVIDER` alongside `ConfigService`. Keep `isGlobal: true` and the `storage.redis` config key unchanged.
  - *Acceptance*: FR-001, FR-020; US1 acceptance scenarios 1–4. Depends on T006, T007.
  - *Live evidence — partial, do not read this as verified.* quickstart §2 was run and the **process survived with the cache degrading as designed**, which is the core of FR-001 and US1. But its two success criteria are **not** carried by that run: **SC-001** is *not established* (its required continuous 10-minute observation window was never run), and **SC-009 FAILED** — every request returned `401 session_store_unavailable` after ~38–42 s, from the separate `ioredis` OIDC session store that is out of scope here and has no `error` listener or tuned timeout. Tracked as [#6332](https://github.com/alkem-io/server/issues/6332); full record in [quickstart.md](./quickstart.md) and [manual-tests-review-fixes.md](./manual-tests-review-fixes.md), and summarised in [plan.md](./plan.md) §"Live verification — actual outcome". The code change below is complete; the acceptance evidence for SC-001/SC-009 is not.
