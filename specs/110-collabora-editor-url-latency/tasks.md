...
- [X] T006 Implement `getLevelZeroSpaceIdForCollaboraDocument(collaboraDocumentId)` in `src/services/infrastructure/entity-resolver/community.resolver.service.ts` using a contribution-first raw scalar query, a framing fallback, and delegation to `getLevelZeroSpaceIdForCalloutsSet`; translate delegated not-found failures to the exact static message `Unable to find Space for CollaboraDocument`, preserve unexpected infrastructure failures, and add the constitution-required leaf-first performance comment without spec, PR, or issue identifiers
