...
| `drift_resolved` | Admin drift-resolve succeeded | `old_email` and `new_email` columns mirror the drift_detected entry; canonical email selected is the one that now equals both sides (admins read the drift_detected entry to know which is which) |
