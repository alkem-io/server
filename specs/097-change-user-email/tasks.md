...
- [ ] T020 [US1] Implement `UserEmailChangeService.applyAdminEmailChange(initiatorAdminId, subjectUserId, newEmail)` in `src/domain/community/user-email-change/user.email.change.service.ts` — happy-path body: load subject user (throw `EMAIL_CHANGE_SUBJECT_NOT_FOUND` if missing) → validate (format already covered by DTO; check no-change against current; check uniqueness on Alkemio + Kratos) → write Kratos with `retryWithBackoff` → write `user.email` in a local TypeORM txn → append `committed` audit entry → invalidate Kratos sessions (wrapped in `retryWithBackoff`; failure → write `session_invalidation_failed` audit entry, but commit stands) → publish the THREE post-commit notification events, each wrapped in `retryWithBackoff` independently (a failure on one MUST NOT prevent the others from being attempted; each writes its own failure-audit on retry-budget exhaustion and commit always stands): (1) `publishEmailChangeSecuritySignal` to OLD address with masked new email — failure → `security_signal_failed` audit entry (FR-016, FR-016a, FR-016b); (2) `publishEmailChangeNewAddressNotification` to NEW address with the full new email and login URL — failure → `new_address_notification_failed` audit entry (FR-016c); (3) `publishEmailChangeGlobalAdminNotification` to the notifications-service fan-out pipeline with `triggerOutcome: COMMITTED` — failure → `global_admin_notification_failed` audit entry (FR-016d) → return `{ success: true, email: newEmail }`. Leave compensating rollback / drift handling stubbed (covered in US2). (FR-002, FR-002a, FR-004, FR-005, FR-006, FR-010, FR-011, FR-016, FR-016a, FR-016b, FR-016c, FR-016d, FR-017, FR-017a)
...
- [ ] T028 [US2] Implement the drift-detection branch in `UserEmailChangeService.applyAdminEmailChange`: if the Kratos-revert step exhausts its retry budget → write `drift_detected` audit entry with `oldEmail = original Alkemio value` and `newEmail = Kratos-observed value`, set `failureReason` to a short non-leaky code (e.g., `kratos_unreachable`), emit Winston `error`-level entry tagged `email_change_drift_detected`, call `apm.captureError(error, { custom: { marker: 'email_change_drift_detected', ... } })`, publish `publishEmailChangeGlobalAdminNotification` with `triggerOutcome: DRIFT_DETECTED` and the observed values (wrapped in `retryWithBackoff`; failure → `global_admin_notification_failed` audit entry; the drift state still stands), throw `EMAIL_CHANGE_DRIFT_DETECTED`. (FR-009a, FR-016d, R5)
- [ ] T029 [US2] Implement `UserEmailChangeService.resolveDrift(adminId, subjectUserId, canonicalEmail)` in `src/domain/community/user-email-change/user.email.change.service.ts`: look up the latest audit entry for the subject (`UserEmailChangeAuditEntryRepository.findLatestBySubject`); if its outcome is NOT `drift_detected` → throw `EMAIL_CHANGE_DRIFT_NOT_FOUND` (a subsequent `drift_resolved` entry would itself be the latest, so this single check covers both "no drift ever" and "drift already resolved"); reject `canonicalEmail` not in `{drift.oldEmail, drift.newEmail}` (validation error); read current Alkemio + Kratos values via `KratosService.getIdentityEmailTrait`; apply alignment writes only where the observed value differs from `canonicalEmail`, each wrapped in `retryWithBackoff`; on success write `drift_resolved` audit entry with `old_email = drift.oldEmail` (mirroring the drift entry's old-side observation) and `new_email = canonicalEmail` (the admin's choice — `canonicalEmail == drift.newEmail` means "keep the change"; `canonicalEmail == drift.oldEmail` means "revert"); on retry exhaustion write `drift_resolution_failed` audit entry and throw `EMAIL_CHANGE_DRIFT_RESOLUTION_FAILED` (FR-009b, see data-model.md §Outcome semantics)
