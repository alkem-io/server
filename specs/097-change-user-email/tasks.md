...
- [ ] T002a [P] Create `PlatformAuditCategory` enum (initially: `EMAIL_CHANGE`; future ISO 27001 categories add values additively per spec.md §FR-014a) in `src/domain/community/user-email-change/enums/platform.audit.category.ts`
- [ ] T003 Create migration `src/migrations/<timestamp>-CreatePlatformAuditEntry.ts` per data-model.md §Migration Plan: 3 Postgres enum types (`platform_audit_category` with single value `'email_change'`; `email_change_audit_outcome` with 11 values; `email_change_initiator_role` with 2 values) + 1 table (`platform_audit_entry`, including `category` enum column and `details: jsonb NULL` column) + 4 indices (PK, row_id, subject+category+created_date, subject+category+row_id) + FK constraints; include reverse-direction `down()`. The table is designed as a platform-wide audit-log foundation (spec.md §FR-014a) — the email-change feature is the FIRST consumer, with all rows written by this spec carrying `category = 'email_change'`.
- [ ] T004 [P] Create `IPlatformAuditEntry` interface in `src/domain/community/user-email-change/platform.audit.entry.interface.ts` mirroring the entity columns from data-model.md §Table 1 (including the `category` and `details` columns)
- [ ] T005 [P] Create `PlatformAuditEntry` TypeORM entity in `src/domain/community/user-email-change/platform.audit.entry.entity.ts` extending `BaseAlkemioEntity` with `@Generated('increment') rowId`; columns per data-model.md §Table 1 (including `category: PlatformAuditCategory` and `details: jsonb NULL`) (depends on T001, T002, T002a, T004)
- [ ] T006 [P] Create `PlatformAuditEntryRepository` provider in `src/domain/community/user-email-change/platform.audit.entry.repository.ts` exposing email-change-scoped read methods (every method implicitly filters `category = 'email_change'`): `appendEmailChangeEntry`, `findEmailChangeBySubjectPaged(cursor, limit)`, `findLatestEmailChangeBySubject(subjectUserId)`, and `findLatestUnresolvedDriftBySubject(subjectUserId)` — the last returns the most recent row with `category = 'email_change' AND outcome = 'drift_detected'` for the subject that has no subsequent `drift_resolved` row, or null. Required by T029 because the simpler "latest entry" heuristic is insufficient when FR-016d post-drift audit writes (`global_admin_notification_failed`) land after the `drift_detected` row. (depends on T005)
...
- [ ] T010 Create audit-write helper service `UserEmailChangeAuditService` in `src/domain/community/user-email-change/user.email.change.service.audit.ts` exposing `record(outcome, { subject, initiator, initiatorRole, oldEmail, newEmail, failureReason? })`. Internally writes a row to `platform_audit_entry` with `category = 'email_change'` (always, in this service); normalises non-leaky failure reasons; leaves `details` JSONB column NULL because the email-change typed columns are sufficient (future-category audit services may use this column instead). (FR-014, FR-014a). Depends on T006.
...
- [ ] T015 Add THREE publish helpers on `NotificationAdapter` (or `NotificationExternalAdapter` per project convention): `publishEmailChangeSecuritySignal(payload)`, `publishEmailChangeNewAddressNotification(payload)`, `publishEmailChangeGlobalAdminNotification(payload)`. Coordinate matching template additions in `@alkemio/notifications-lib` (3 new templates) and bump dependency version. The third helper reuses the same Global-Role-Change fan-out pipeline on the notifications-service side; the server does NOT enumerate admin recipients.
...
- [ ] T028 [US2] Implement the drift-detection branch in `UserEmailChangeService.applyAdminEmailChange`. Sequence (order matters — the `drift_detected` row MUST be persisted FIRST so T029's `findLatestUnresolvedDriftBySubject` lookup correctly identifies the outstanding drift even when a subsequent `global_admin_notification_failed` row is appended): (1) if the Kratos-revert step exhausts its retry budget → write `drift_detected` audit entry with `oldEmail = original Alkemio value` and `newEmail = Kratos-observed value`, set `failureReason` to a short non-leaky code (e.g., `kratos_unreachable`); (2) emit Winston `error`-level entry tagged `email_change_drift_detected`; (3) call `apm.captureError(error, { custom: { marker: 'email_change_drift_detected', ... } })`; (4) publish `publishEmailChangeGlobalAdminNotification` with `triggerOutcome: DRIFT_DETECTED` and the observed values (wrapped in `retryWithBackoff`; on retry-budget exhaustion → append `global_admin_notification_failed` audit entry after the drift entry; the drift state still stands and remains discoverable via T029 because the drift entry has no subsequent `drift_resolved` row); (5) throw `EMAIL_CHANGE_DRIFT_DETECTED`. (FR-009a, FR-016d, R5)
- [ ] T029 [US2] Implement `UserEmailChangeService.resolveDrift(adminId, subjectUserId, canonicalEmail)` in `src/domain/community/user-email-change/user.email.change.service.ts`: look up the most recent `drift_detected` audit entry for the subject AND verify no subsequent `drift_resolved` audit entry exists for that subject — this requires a slightly richer query than `findLatestBySubject`, e.g., `findLatestUnresolvedDriftBySubject` on the repository. The simpler "latest audit entry, must be `drift_detected`" heuristic is INSUFFICIENT because FR-016d's GA-publish-failure path (T028) can write a `global_admin_notification_failed` audit entry AFTER the `drift_detected` row, making the latest entry not be `drift_detected` even when the drift remains outstanding. If no such unresolved drift row exists → throw `EMAIL_CHANGE_DRIFT_NOT_FOUND`. Then: reject `canonicalEmail` not in `{drift.oldEmail, drift.newEmail}` (validation error); read current Alkemio + Kratos values via `KratosService.getIdentityEmailTrait`; apply alignment writes only where the observed value differs from `canonicalEmail`, each wrapped in `retryWithBackoff`; on success write `drift_resolved` audit entry with `old_email = drift.oldEmail` (mirroring the drift entry's old-side observation) and `new_email = canonicalEmail` (the admin's choice — `canonicalEmail == drift.newEmail` means "keep the change"; `canonicalEmail == drift.oldEmail` means "revert"); on retry exhaustion write `drift_resolution_failed` audit entry and throw `EMAIL_CHANGE_DRIFT_RESOLUTION_FAILED` (FR-009b, see data-model.md §Outcome semantics). Also extend T006's repository to expose `findLatestUnresolvedDriftBySubject(subjectUserId)`.
