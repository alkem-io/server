...
- [ ] T003 Create migration `src/migrations/<timestamp>-CreateEmailChangeAuditEntry.ts` per data-model.md §Migration Plan: 2 Postgres enum types (`email_change_audit_outcome` with 11 values, `email_change_initiator_role` with 2 values) + 1 table (`email_change_audit_entry`) + 3 indices + FK constraints; include reverse-direction `down()`
...
- [ ] T006 [P] Create `UserEmailChangeAuditEntryRepository` provider in `src/domain/community/user-email-change/user.email.change.audit.entry.repository.ts` exposing `appendEntry`, `findBySubjectPaged(cursor, limit)`, `findLatestBySubject(subjectUserId)`, and `findLatestUnresolvedDriftBySubject(subjectUserId)` — the last returns the most recent `drift_detected` audit row for the subject that has no subsequent `drift_resolved` row, or null. Required by T029 because the simpler "latest entry" heuristic is insufficient when FR-016d post-drift audit writes (`global_admin_notification_failed`) land after the `drift_detected` row. (depends on T005)
