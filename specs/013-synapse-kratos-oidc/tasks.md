...
1. **Synapse OIDC Discovery vs. Explicit Endpoints**: Initial attempt used `discover: true` (autodiscovery) which created hard startup dependencies - Synapse failed to start if Hydra wasn't ready. **Solution**: `discover: false` with explicit hybrid endpoint configuration: public URLs for browser-facing authorization, internal Docker URLs for backend (token/userinfo/jwks). See `specs/013-synapse-kratos-oidc/retrospective.md` for complete analysis.
...
**DOCUMENTATION**: Comprehensive environment variables reference created at `specs/013-synapse-kratos-oidc/environment-variables.md`
...
- [x] T034 Document troubleshooting procedures in `specs/013-synapse-kratos-oidc/quickstart.md` (based on testing findings)
...
1. `specs/013-synapse-kratos-oidc/retrospective.md` - Complete retrospective of Synapse integration challenges and solutions
2. `specs/013-synapse-kratos-oidc/SYNAPSE_OIDC_CONFIGURATION.md` - Configuration guide and operational procedures
