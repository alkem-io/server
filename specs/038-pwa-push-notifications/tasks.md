...
- [x] T011 Push notifications use `@golevelup/nestjs-rabbitmq` `@RabbitSubscribe` decorator in PushDeliveryService — no `connectMicroservice` call needed. A NestJS Transport.RMQ consumer would compete with the golevelup handler and silently drop messages. Comment added in src/main.ts (line ~106-112) explaining this decision.
...
**Goal**: Subscribed users receive push notifications on their device when any of the ~34 platform events fire (including 4 poll notification events). Per-user throttling enforced at 10/min via Redis. Transient delivery failures are logged and dropped (fire-and-forget; next platform event reattempts naturally). Expired subscriptions auto-cleaned on 410 Gone response.
...
- [x] T023 [P] [US2] Unit test for PushDeliveryService: test successful delivery (mock web-push.sendNotification resolves), 410 Gone handling (calls PushSubscriptionService.markExpired + logs), transient error handling (ack and drop — fire-and-forget), lastActiveDate update on success in src/services/adapters/notification-push-adapter/push.delivery.service.spec.ts
...
- [x] T026 [P] [US2] Create PushDeliveryService: consume from push-notifications queue via `@RabbitSubscribe`, call web-push.sendNotification(endpoint, payload, {vapidDetails}), on success update subscription.lastActiveDate + log, on 410 Gone call PushSubscriptionService.markExpired + log, on transient error ack and drop (fire-and-forget — next platform event reattempts naturally) + log in src/services/adapters/notification-push-adapter/push.delivery.service.ts
...
- [x] T031 [US2] Add `this.notificationPushAdapter.sendPushNotifications(recipients.pushRecipients, event, payload)` call in every event handler that calls sendInAppNotifications() across all 5 domain notification adapters: NotificationSpaceAdapter (13 handlers, including poll notifications), NotificationUserAdapter (7 handlers), NotificationPlatformAdapter (6 handlers), NotificationOrganizationAdapter (3 handlers), NotificationVirtualContributorAdapter (1 handler) — total 30 call sites. Note: 2 handlers (in Space and Platform) that only call sendExternalNotifications without sendInAppNotifications are intentionally excluded — push mirrors inApp behavior per FR-002. For each call site, construct the push payload `{title, body, url}` using entity names and IDs already loaded in the handler. Title: short action summary (e.g., "New comment on {spaceName}"). Body: one-line preview, reusing email subject text pattern, truncated to 200 chars. URL: relative deep link path per T032. Files in src/services/adapters/notification-adapter/

**Checkpoint**: Push notifications delivered to subscribed users when platform events fire (including poll vote/modification events). Throttling limits to 10/min/user. Transient failures are logged and dropped. Expired subscriptions auto-cleaned on 410.
...
- [x] T040 [P] Add push notification dispatch to `sendPollNotification()` method in NotificationSpaceAdapter — was missing after merge from develop which introduced poll notification events. Constructs push payload with poll title and space name, uses `UrlGeneratorService.getCalloutUrlPath()` for deep link URL. File: src/services/adapters/notification-adapter/notification.space.adapter.ts
- [x] T041 [P] Generate migration `AddPushFieldToPollNotificationSettings` to backfill `push` field on 4 poll notification settings in user_settings JSONB (collaborationPollVoteCastOnOwnPoll, collaborationPollVoteCastOnPollIVotedOn, collaborationPollModifiedOnPollIVotedOn, collaborationPollVoteAffectedByOptionChange) — poll settings were added by CommunityPolls migration with only `{email, inApp}`. File: src/migrations/1774254508094-AddPushFieldToPollNotificationSettings.ts
- [x] T042 [P] Update poll notification defaults in `getDefaultUserSettings()` to include `push: false` and fix test fixtures (user.settings.service.spec.ts, user.settings.mock.ts) to include `push` on poll notification entries
