...
- Q: What replaces the delta-only approach? → A: Shared Inherited Credential Rule Sets. Each policy's credential rules are split into local (entity-specific) and inherited (cascading from ancestors). Inherited rule sets are deduplicated by parent ownership and stored in a shared lookup table. Each policy holds an eager-loaded foreign key to its inherited rule set, so authorization checks require zero additional queries.
