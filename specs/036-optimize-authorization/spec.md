...
**Storage math**: In a typical Space tree, most entities share one of ~64 distinct inherited rule set combinations (varying by tree depth and Space privacy mode). With ~1000 policies in a medium deployment, replacing ~1000 full JSONB blobs with ~64 shared rows achieves an estimated **~81% storage reduction** while adding zero runtime query overhead.
