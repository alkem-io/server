...
- PostgreSQL 17.5; framing Collabora document persisted via existing `collabora_document` table; bytes via existing storage subsystem and file-service (Go service exposing the upload/sniff/validate API used today by `importCollaboraDocument`) (095-collabora-import)
...
- TypeScript 5.3 on Node.js 22 LTS (Volta-pinned 22.21.1) + NestJS 10, Apollo Server 4 / GraphQL 16, `ioredis@5.10.1`, `express-session@1.19` / `connect-redis@7.1`. Every `ioredis` client MUST be built through `src/core/redis/redis.client.factory.ts` — never `new Redis()` directly. The factory's fail-fast options (`enableOfflineQueue: false`, `commandTimeout: 500`, `connectTimeout: 500`, `maxRetriesPerRequest: 1`) are behavioural constants, deliberately not configuration: they are what stops a Redis outage becoming a ~42 s hang. (story/6332-redis-session-store-resilience)
- Redis only, and only for connection behaviour — the `alkemio:sid:<sid>` and `alkemio:sub:<sub>` keyspaces are untouched. No entity, no migration, no GraphQL schema change. A request carrying no session cookie must issue zero session-store commands; store-unreachable surfaces as 503 + `Retry-After` (cookie re-asserted, never cleared) on every transport, never as 401. (story/6332-redis-session-store-resilience)
