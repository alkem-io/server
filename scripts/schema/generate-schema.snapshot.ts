...
async function main() {
  ...
  try {
    ...
    if (forceSuccess) {
      ...
      process.exitCode = 0;
      // Emit a minimal placeholder SDL so downstream diffing & gating steps still operate.
      if (!wroteSnapshot) {
        try {
          const placeholder =
            '# Placeholder SDL generated due to force-success after failure\n\n"""Autogenerated placeholder root query to keep CI pipeline progressing."""\n type Query { _placeholder: Boolean }\n';
          writeFileSync(outPath, placeholder, 'utf-8');
          process.stderr.write(
            '[force-success] Wrote placeholder schema to ' + outPath + '\n'
          );
        } catch (writeErr) {
          process.stderr.write(
            '[force-success] Failed to write placeholder schema: ' +
