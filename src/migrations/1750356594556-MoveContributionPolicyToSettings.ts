...
export class MoveContributionPolicyToSettings1750356594556
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`callout_settings\` ADD \`contributionId\` char(36) NULL`
    );
    ...
    await queryRunner.query(
      `CREATE UNIQUE INDEX \`REL_03f9b3e126314ef81473ced556\` ON \`callout_settings\` (\`contributionId\`)`
    );
    await queryRunner.query(
      `ALTER TABLE \`callout_settings\` ADD CONSTRAINT \`FK_03f9b3e126314ef81473ced5568\` FOREIGN KEY (\`contributionId\`) REFERENCES \`callout_settings_contribution\`(\`id\`) ON DELETE SET NULL ON UPDATE NO ACTION`
    );

    await queryRunner.query(
      `UPDATE \`callout_settings_contribution\` SET \`canAddContributions\` = 'none'`
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`callout_settings\` DROP FOREIGN KEY \`FK_03f9b3e126314ef81473ced5568\``
    );
    await queryRunner.query(
      `DROP INDEX \`REL_03f9b3e126314ef81473ced556\` ON \`callout_settings\``
    );
    ...
    await queryRunner.query(
      `ALTER TABLE \`callout_settings\` DROP COLUMN \`contributionId\``
    );
