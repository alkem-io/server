...
export class innovationSpace1680171984719 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`innovation_space\` (\`id\` varchar(36) NOT NULL, \`createdDate\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedDate\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`version\` int NOT NULL, \`nameID\` varchar(255) NOT NULL, \`type\` varchar(255) NOT NULL, \`authorizationId\` varchar(36) NULL, \`profileId\` varchar(36) NULL, \`selectionCriteriaId\` varchar(36) NULL, \`brandingId\` varchar(36) NULL, UNIQUE INDEX \`REL_69503638dad11b28081aa11a87\` (\`authorizationId\`), UNIQUE INDEX \`REL_1dfeddbcbd97e3a052b62bbbc3\` (\`profileId\`), UNIQUE INDEX \`REL_d0ee4040ea8ff06d269c0507da\` (\`selectionCriteriaId\`), UNIQUE INDEX \`REL_c7f7ab0964b33595ba98513ea8\` (\`brandingId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`
    );
    await queryRunner.query(
      `ALTER TABLE \`selection_filter\` ADD CONSTRAINT \`FK_34b78059a6841d5e97c3d787079\` FOREIGN KEY (\`selectionCriteriaId\`) REFERENCES \`selection_criteria\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`innovation_space\` ADD CONSTRAINT \`FK_d0ee4040ea8ff06d269c0507da2\` FOREIGN KEY (\`selectionCriteriaId\`) REFERENCES \`selection_criteria\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`
    );
    await queryRunner.query(
