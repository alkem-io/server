 ...
 */
export class NotificationEntityTracking1760357366405
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` ADD \`spaceID\` char(36) NULL COMMENT 'FK to Space - cascade deletes notification when space is deleted'`
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` ADD \`organizationID\` char(36) NULL COMMENT 'FK to Organization - cascade deletes notification when organization is deleted'`
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` ADD \`userID\` char(36) NULL COMMENT 'FK to User - cascade deletes notification when referenced user is deleted'`
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` ADD \`applicationID\` char(36) NULL COMMENT 'FK to Application - cascade deletes notification when application is deleted'`
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` ADD \`invitationID\` char(36) NULL COMMENT 'FK to Invitation - cascade deletes notification when invitation is deleted'`
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` ADD \`calloutID\` char(36) NULL COMMENT 'FK to Callout - cascade deletes notification when callout is deleted'`
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` ADD \`contributionID\` char(36) NULL COMMENT 'FK to Callout Contribution - cascade deletes notification when Contribution is deleted'`
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` ADD \`roomID\` char(36) NULL COMMENT 'FK to Room - cascade deletes notification when the room is deleted'`
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` ADD \`messageID\` char(44) NULL COMMENT 'Not actual FK - used to manually delete notification when the message is deleted'`
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` ADD \`contributorOrganizationID\` char(36) NULL COMMENT 'FK to Organization - cascade deletes notification when organization contributor is deleted'`
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` ADD \`contributorUserID\` char(36) NULL COMMENT 'FK to User - cascade deletes notification when user contributor is deleted'`
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` ADD \`contributorVcID\` char(36) NULL COMMENT 'FK to VC - cascade deletes notification when VC contributor is deleted'`
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` ADD \`calendarEventID\` char(36) NULL COMMENT 'FK to Calendar Event - cascade deletes notification when the calendar event is deleted'`
    );
    ...
    const data: { id: string; contributionParentID: string }[] =
      await queryRunner.query(`
          ...
          WHERE JSON_UNQUOTE(payload->'$.type') = 'SPACE_COLLABORATION_CALLOUT_POST_COMMENT' AND callout_contribution.id IS NOT NULL;
      `);
    ...
    for (const { notificationID, userID, orgID, vcID } of result) {
      if (userID) {
        ...
        console.warn(
          `No matching contributor entity found for notification '${notificationID}' with contributorID in payload.`
        );
      }
    }
    await queryRunner.query(`
    ...
    await queryRunner.query(`
          UPDATE in_app_notification
          SET calendarEventID = JSON_UNQUOTE(JSON_EXTRACT(payload, '$.calendarEventID'))
          WHERE JSON_EXTRACT(payload, '$.calendarEventID') IS NOT NULL
    ...
    await queryRunner.query(`
          DELETE in_app_notification FROM in_app_notification
          LEFT JOIN calendar_event ON in_app_notification.calendarEventID = calendar_event.id
          WHERE in_app_notification.calendarEventID IS NOT NULL AND calendar_event.id IS NULL
      `);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP FOREIGN KEY \`FK_8bf8b7bba9aab93abcaa4238820\``
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP FOREIGN KEY \`FK_c9c5d92154e4300bad54b7bbcc7\``
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP FOREIGN KEY \`FK_e4b8c5447b138bd2ce749274ae3\``
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP FOREIGN KEY \`FK_618449b41643adab8598b80e1b2\``
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP FOREIGN KEY \`FK_439dd686c1912533c380b783f0b\``
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP FOREIGN KEY \`FK_6df3d947b625cf6bd2ed856f632\``
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP FOREIGN KEY \`FK_75c3fa6ba71954e8586bfdbe725\``
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP FOREIGN KEY \`FK_b2f1dc00232220031a6921da1b9\``
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP FOREIGN KEY \`FK_b8fe43c84d0f765bba5f6bd054d\``
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP FOREIGN KEY \`FK_d298041d567d984ed6c0667c814\``
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP FOREIGN KEY \`FK_3a71f82d91a3809bd652cd80f1f\``
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP FOREIGN KEY \`FK_60085ab32808bc5f628fe3ca587\``
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP FOREIGN KEY \`FK_a84dd5170304562dbd58b37521e\``
    );
    // Drop all columns
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP COLUMN \`calendarEventID\``
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP COLUMN \`messageID\``
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP COLUMN \`roomID\``
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP COLUMN \`contributionID\``
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP COLUMN \`calloutID\``
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP COLUMN \`invitationID\``
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP COLUMN \`applicationID\``
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP COLUMN \`userID\``
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP COLUMN \`organizationID\``
    );
    await queryRunner.query(
      `ALTER TABLE \`in_app_notification\` DROP COLUMN \`spaceID\``
    );
  }
}
