...
export class whiteboardToRt1708354354175 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `UPDATE whiteboard SET contentUpdatePolicy = 'admins'`
    );
    // update all the callouts with whiteboardRT type
    await queryRunner.query(
      `UPDATE \`callout\` SET type = 'whiteboard' WHERE type = 'whiteboard-real-time'`
    );
    // update all callout templates
    await queryRunner.query(
      `UPDATE \`callout_template\` SET type = 'whiteboard' WHERE type = 'whiteboard-real-time'`
    );
    // rename feature flag
    await queryRunner.query(
