...
/**
 * Backfills the two new user-side organization-associate notification rows
 * (`organizationAssociateInvitationReceived`,
 * `organizationAssociateApplicationDecided`) onto every existing
 * `user_settings` row, at the mandated defaults
 * `{ email: true, inApp: true, push: true }`. Cloned from
 ...
 */
export class AddUserOrganizationAssociateNotificationSettings1788800000000
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    const value =
      AddUserOrganizationAssociateNotificationSettings1788800000000
        .DEFAULT_VALUE;
    for (const key of [
      'organizationAssociateInvitationReceived',
      ...
      await queryRunner.query(
        ...
        WHERE notification #> ('{user,membership,' || $2 || '}')::text[] IS NULL
        `,
        [value, key]
      );
    }
