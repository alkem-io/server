...
export class canvas1629887578891 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      'CREATE TABLE `alkemio`.`canvas` (`id` char(36) NOT NULL, `createdDate` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `updatedDate` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), `version` int NOT NULL, `name` text NOT NULL, `value` text NOT NULL, PRIMARY KEY (`id`)) ENGINE=InnoDB'
    );
    await queryRunner.query(
      'ALTER TABLE `alkemio`.`ecosystem_model` ADD `canvasId` char(36) NULL'
    );
    await queryRunner.query(
      'ALTER TABLE `alkemio`.`ecosystem_model` ADD UNIQUE INDEX `IDX_c9ed67519d26140f98265a542e` (`canvasId`)'
    );
    await queryRunner.query(
