...
export class AccountService {
  ...
  async accountExists(accountUpn: string): Promise<boolean> {
    ...
    return await this.identityService.userExists(accountUpn);
  }
  ...
  async createUserAccount(userData: UserInput): Promise<boolean> {
    ...
    const result = await this.identityService.createUser(userData, accountUpn);
    if (!result)
      throw new AccountException(
  ...
  async removeUserAccount(accountUpn: string): Promise<boolean> {
    ...
    let res = false;
    try {
      res = await this.identityService.removeUser(accountUpn);
    } catch (error) {
      throw new AccountException(
  ...
  async updateUserAccountPassword(
    ...
    let res = false;
    try {
      res = await this.identityService.updateUserPassword(
        accountUpn,
        newPassword
      );
    } catch (error) {
