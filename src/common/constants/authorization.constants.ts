export const CREDENTIAL_RULE_CHALLENGE_HUB_ADMINS = 'challengeHubAdmins';
export const CREDENTIAL_RULE_CHALLENGE_ADMINS = 'challengeAdmins';
export const CREDENTIAL_RULE_CHALLENGE_MEMBER_READ = 'challengeMemberRead';
export const CREDENTIAL_RULE_CHALLENGE_INNOVATION_FLOW =
  'challengeInnovationFlow';
export const CREDENTIAL_RULE_CHALLENGE_CREATE_OPPORTUNITY =
  'challengeCreateOpportunity';
export const CREDENTIAL_RULE_CHALLENGE_HUB_MEMBER_JOIN =
  'challengeHubMemberJoin';
export const CREDENTIAL_RULE_CHALLENGE_HUB_MEMBER_APPLY =
  'challengeHubMemberApply';
export const CREDENTIAL_RULE_CHALLENGE_HUB_ADMIN_DELETE =
  'challengeHubAdminDelete';
export const CREDENTIAL_RULE_HUB_AUTHORIZATION_RESET = 'hubAuthorizationReset';
export const CREDENTIAL_RULE_HUB_GLOBAL_ADMIN_COMMUNITY_READ =
  'hubGlobalAdminCommunityRead';
export const CREDENTIAL_RULE_HUB_AUTHORIZATION_GLOBAL_ADMIN_GRANT =
  'hubAuthorizationGlobalAdminGrant';
export const CREDENTIAL_RULE_HUB_CREATE_CHALLENGE = 'hubCreateChallenge';
export const CREDENTIAL_RULE_HUB_ADMINS = 'hubAdmins';
export const CREDENTIAL_RULE_HUB_MEMBERS_CREATE_CHALLENGES =
  'hubMembersCreateChallenges';
export const CREDENTIAL_RULE_HUB_MEMBERS_READ = 'hubMembersRead';
export const CREDENTIAL_RULE_HUB_COMMUNITY_APPLY_GLOBAL_REGISTERED =
  'hubCommunityApplyGlobalRegistered';
export const CREDENTIAL_RULE_HUB_COMMUNITY_JOIN_GLOBAL_REGISTERED =
  'hubCommunityJoinGlobalRegistered';
export const CREDENTIAL_RULE_HUB_HOST_ASSOCIATES_JOIN = 'hubHostAssociatesJoin';
export const CREDENTIAL_RULE_ASPECT_CREATED_BY = 'aspectCreatedBy';
export const CREDENTIAL_RULE_ASPECT_ADMINS_MOVE_CARD = 'aspectAdminsMoveCard';
