...
describe('FileServiceAdapter', () => {
  ...
  describe('getDocumentMetaBatch (best-effort, isolated from the shared breaker)', () => {
    const metaFile = (id: string, width: number, height: number) => ({
      id,
      externalID: `ext-${id}`,
      storageBucketId: 'bucket-1',
      mimeType: 'image/png',
    ...
    it('issues ONE direct POST to /internal/file/meta-batch with the SHORT timeout — bypassing sendRequest/the breaker — and keys the answer by id', async () => {
      const files = [metaFile('doc-a', 10, 20), metaFile('doc-b', 30, 40)];
      (httpService.post as Mock).mockReturnValue(of(axiosResponse({ files })));

      const result = await adapter.getDocumentMetaBatch(['doc-a', 'doc-b']);

      // Direct httpService.post (NOT the breaker-routed request pipeline).
      expect(httpService.request).not.toHaveBeenCalled();
      expect(httpService.post).toHaveBeenCalledTimes(1);
      const [url, body, options] = (httpService.post as Mock).mock.calls[0];
      expect(url).toBe('http://file-service:4003/internal/file/meta-batch');
      expect(body).toEqual({ ids: ['doc-a', 'doc-b'] });
      // SHORT per-call timeout (DOCUMENT_META_TIMEOUT_MS), no retries config.
      expect(options).toEqual({ timeout: 2500 });

      expect([...result.keys()]).toEqual(['doc-a', 'doc-b']);
      expect(result.get('doc-a')).toEqual(files[0]);
      expect(result.get('doc-b')).toEqual(files[1]);
    });

    it('treats an UNRESOLVED id as a normal partial 200 — it is simply absent from the map (order is not relied on)', async () => {
      // Contract: ids that do not resolve are OMITTED, and response order is not
      // guaranteed. Answer out of order and one short.
      (httpService.post as Mock).mockReturnValue(
        of(axiosResponse({ files: [metaFile('doc-c', 5, 6)] }))
      );

      const result = await adapter.getDocumentMetaBatch([
        'doc-a',
        'doc-b',
        'doc-c',
      ]);
      ...
      expect(result.get('doc-c')).toMatchObject({
        imageWidth: 5,
        imageHeight: 6,
      });
      expect(result.has('doc-a')).toBe(false);
      expect(result.has('doc-b')).toBe(false);
    });
    ...
    it('de-duplicates repeated ids (the answer is keyed by id; a repeat only eats the 100-id budget)', async () => {
      (httpService.post as Mock).mockReturnValue(
        of(axiosResponse({ files: [metaFile('doc-a', 1, 2)] }))
      );

      await adapter.getDocumentMetaBatch(['doc-a', 'doc-a', 'doc-a']);

      expect((httpService.post as Mock).mock.calls[0][1]).toEqual({
        ids: ['doc-a'],
      });
    });

    it('CHUNKS at 100 ids (file-service 400s beyond that) and merges the chunk answers', async () => {
      const ids = Array.from({ length: 250 }, (_, i) => `doc-${i}`);
      (httpService.post as Mock).mockImplementation(
        (_url: string, body: { ids: string[] }) =>
          of(
            axiosResponse({
              files: body.ids.map(id => metaFile(id, 1, 1)),
            })
          )
      );

      const result = await adapter.getDocumentMetaBatch(ids);

      expect(httpService.post).toHaveBeenCalledTimes(3);
      const sentChunks = (httpService.post as Mock).mock.calls.map(
        call => call[1].ids as string[]
      );
      expect(sentChunks.map(chunk => chunk.length)).toEqual([100, 100, 50]);
      expect(sentChunks.flat()).toEqual(ids);
      expect(result.size).toBe(250);
    });

    it('returns an EMPTY map on 404 (a file-service predating this route) — QUIETLY, without logging', async () => {
      const axiosError = new AxiosError('Not Found', '404', undefined, null, {
        status: 404,
        data: { error: 'not found' },
        statusText: 'Not Found',
      ...
      const result = await adapter.getDocumentMetaBatch(['doc-1']);

      expect(result.size).toBe(0);
      // Rollout-quiet: a WARN per message read would be pure spam.
      expect(mockLogger.warn).not.toHaveBeenCalled();
    });

    it('swallows EVERY other failure (400 / 5xx / timeout / network) to an empty map — never propagates — and logs a WARN', async () => {
      const failures = [
        // 400 — malformed request (non-UUID id, or >100 ids)
        new AxiosError('Bad Request', '400', undefined, null, {
          status: 400,
      ...
      for (const err of failures) {
        (httpService.post as Mock).mockReturnValue(throwError(() => err));
        mockLogger.warn.mockClear();

        const result = await adapter.getDocumentMetaBatch(['doc-1', 'doc-2']);
        expect(result.size).toBe(0);
        ...
        expect(payload).toMatchObject({
          message: expect.stringContaining(
            'Best-effort batched document meta lookup failed'
          ),
          documentIdCount: 2,
          error: err.message,
        });
    ...
    it('tolerates a malformed body (no `files` array) as "no measurements" rather than throwing', async () => {
      (httpService.post as Mock).mockReturnValue(
        of(axiosResponse({ unexpected: true } as any))
      );

      await expect(adapter.getDocumentMetaBatch(['doc-1'])).resolves.toEqual(
        new Map()
      );
    });
    ...
    it('does NOT gate on the shared circuit breaker — still fetches even when the breaker is OPEN', async () => {
      ...
      // getDocumentMetaBatch ignores the open breaker: it STILL issues its POST.
      (httpService.post as Mock).mockReturnValue(
        of(axiosResponse({ files: [metaFile('doc-1', 4, 2)] }))
      );
      const result = await adapter.getDocumentMetaBatch(['doc-1']);
    ...
    it('failing batch fetches never trip the shared breaker guarding uploads/pins', async () => {
      const err500 = new AxiosError('Boom', '500', undefined, null, {
        status: 500,
        data: { error: 'internal' },
        statusText: 'Internal Server Error',
      ...
      (httpService.post as Mock).mockReturnValue(throwError(() => err500));

      // Ten failed batch fetches — far past the breaker threshold if they counted.
      for (let i = 0; i < 10; i++) {
        const result = await adapter.getDocumentMetaBatch([`doc-${i}`]);
        expect(result.size).toBe(0);
      }
    ...
    it('returns an empty map when the adapter is disabled (best-effort, does not throw)', async () => {
      const disabledModule = await Test.createTestingModule({
        providers: [
          FileServiceAdapter,
          {
            provide: HttpService,
            useValue: { request: vi.fn(), get: vi.fn(), post: vi.fn() },
          },
          {
            provide: ConfigService,
            useValue: {
      ...
      await expect(
        disabledAdapter.getDocumentMetaBatch(['doc-1'])
      ).resolves.toEqual(new Map());
      expect(disabledHttp.post).not.toHaveBeenCalled();
    });
  });
