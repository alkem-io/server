 ...
 *
 * In schema bootstrap mode (SCHEMA_BOOTSTRAP_LIGHT=1), this module
 * uses a stub to avoid requiring RabbitMQ infrastructure.
 */

// Schema bootstrap mode - use stub to avoid RabbitMQ dependency
const isSchemaBootstrap = process.env.SCHEMA_BOOTSTRAP_LIGHT === '1';

const rabbitMqImport = RabbitMQModule.forRootAsync({
  imports: [ConfigModule],
  inject: [ConfigService],
  useFactory: async (configService: ConfigService<AlkemioConfig, true>) => {
    const rbmqConfig = configService.get('microservices.rabbitmq.connection', {
      infer: true,
    });
