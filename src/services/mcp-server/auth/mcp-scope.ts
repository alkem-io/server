 ...
 *
 * - `undefined` scopes => the caller did not authenticate with an MCP API key
 ...
 * - An explicit empty array `[]` => an authenticated key that was granted NO
 *   operations. This fails closed: every operation is denied (it must never be
 *   conflated with "no key" and silently bypass scope enforcement).
...
export const scopeViolation = (
  ...
  if (scopes.length === 0) {
    return 'This API key has no granted operations.';
  }
