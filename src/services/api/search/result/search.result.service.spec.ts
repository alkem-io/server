...
describe('SearchResultService', () => {
  ...
  let organizationLookupService: { organizationsWithCredentials: Mock };
  let entityManager: { findBy: Mock; find: Mock; findOneByOrFail: Mock; findOne: Mock };
  ...
  describe('getSpaceSearchResults', () => {
    ...
    it('should return single space result when spaceId filter is provided', async () => {
      const rawResults = [
        { id: 'sr-1', score: 5, type: SearchResultType.SPACE, result: { id: 'space-1' } },
      ] as any[];
    ...
    it('should map raw results to spaces found by entity manager', async () => {
      const rawResults = [
        { id: 'sr-1', score: 5, type: SearchResultType.SPACE, result: { id: 'space-1' } },
        { id: 'sr-2', score: 3, type: SearchResultType.SPACE, result: { id: 'space-2' } },
      ] as any[];
      const spaces = [
        { id: 'space-1' },
        { id: 'space-2' },
  ...
  describe('getSubspaceSearchResults', () => {
    ...
    it('should filter out subspaces user is not authorized to read', async () => {
      const rawResults = [
        { id: 'sr-1', score: 5, type: SearchResultType.SUBSPACE, result: { id: 'sub-1' } },
      ] as any[];
      entityManager.find.mockResolvedValue([
        { id: 'sub-1', authorization: { id: 'auth-1' }, parentSpace: { id: 'space-1' } },
      ]);
      authorizationService.isAccessGranted.mockReturnValue(false);

      const result = await service.getSubspaceSearchResults(rawResults, agentInfo);
    ...
    it('should filter out subspaces without parentSpace', async () => {
      ...
      entityManager.find.mockResolvedValue([
        { id: 'sub-1', authorization: { id: 'auth-1' }, parentSpace: undefined },
      ]);
  ...
  describe('getUserSearchResults', () => {
    ...
    it('should filter users by space membership when spaceId is provided', async () => {
      const rawResults = [
        { id: 'sr-1', score: 5, type: SearchResultType.USER, result: { id: 'user-1' } },
        { id: 'sr-2', score: 3, type: SearchResultType.USER, result: { id: 'user-2' } },
      ] as any[];
  ...
  describe('getOrganizationSearchResults', () => {
    ...
    it('should filter out organizations without READ authorization', async () => {
      const rawResults = [
        { id: 'sr-1', score: 5, type: SearchResultType.ORGANIZATION, result: { id: 'org-1' } },
      ] as any[];
      ...
      const result = await service.getOrganizationSearchResults(rawResults, agentInfo);
