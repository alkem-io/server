 ...
 *  - test.pdf   : single-page PDF, body text "atlas voyager pdf archive text"
...
describe('CollaboraTextExtractService', () => {
  ...
  describe('extract (FR-001/014/016/018)', () => {
    ...
    it('extracts pdf body text — PDF is a first-class Collabora document type, not out of scope', async () => {
      const moduleRef = await buildService({
        getDocumentContent: async () => bufferFor('pdf'),
      });
      const service = moduleRef.get(CollaboraTextExtractService);

      const { content, skipReason } = await service.extract(
        docEntity(1000, 'cd-1', MimeTypeDocument.PDF)
      );
      expect(skipReason).toBeUndefined();
      expect(content).toContain('atlas voyager pdf archive text');
    });
