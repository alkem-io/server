...
describe('SearchIngestService', () => {
  ...
  describe('ingestFromScratch', () => {
    ...
    it('should completeWithError and log error if an error is thrown', async () => {
      ...
      // Use direct assignment for mock objects
      const completeWithErrorSpy = vi.fn().mockResolvedValue(undefined);
      mockTaskService.completeWithError = completeWithErrorSpy;
      const logSpy = vi.spyOn(mockLogger, 'error').mockImplementation(() => {});
    ...
    it('should complete the task and log success when all steps succeed', async () => {
      ...
      vi.spyOn(service as any, 'ingestStepCreateIndices').mockImplementation(
        async () => {}
      );
      vi.spyOn(
        service as any,
        'ingestStepIngestIntoIndices'
      ).mockImplementation(async () => {});
      vi.spyOn(service as any, 'ingestStepAssignAliases').mockImplementation(
        async () => []
      );
