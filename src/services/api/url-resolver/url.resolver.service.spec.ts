...
describe('UrlResolverService', () => {
  ...
  let spaceLookupService: { getSpaceByNameIdOrFail: Mock; getSubspaceByNameIdInLevelZeroSpace: Mock; getSpaceOrFail: Mock };
  let innovationHubService: { getInnovationHubByNameIdOrFail: Mock };
  let innovationPackService: { getInnovationPackByNameIdOrFail: Mock };
  let forumDiscussionLookupService: { getForumDiscussionByNameIdOrFail: Mock };
  let virtualContributorLookupService: { getVirtualContributorByNameIdOrFail: Mock };
  let urlGeneratorService: { generateUrlForPlatform: Mock; getSpaceUrlPathByID: Mock; generateUrlForVCById: Mock };
  ...
  beforeEach(async () => {
    ...
    forumDiscussionLookupService = module.get(ForumDiscussionLookupService) as any;
  ...
  describe('resolveUrl', () => {
    it('should return HOME type when URL path is empty', async () => {
      const result = await service.resolveUrl('https://example.com/', agentInfo);
    ...
    it('should return HOME type for /home base route', async () => {
      const result = await service.resolveUrl('https://example.com/home', agentInfo);

      expect(result.type).toBe(UrlType.HOME);
    });

    it('should return FLOW type for /create-space route', async () => {
      const result = await service.resolveUrl('https://example.com/create-space', agentInfo);

      expect(result.type).toBe(UrlType.FLOW);
    });

    it('should return DOCUMENTATION type for /docs route', async () => {
      const result = await service.resolveUrl('https://example.com/docs', agentInfo);
    ...
    it('should resolve user URL with user ID', async () => {
      ...
      const result = await service.resolveUrl('https://example.com/user/john-doe', agentInfo);

      expect(result.type).toBe(UrlType.USER);
      expect(result.userId).toBe('user-uuid-1');
      expect(userLookupService.getUserByNameIdOrFail).toHaveBeenCalledWith('john-doe');
    });
    ...
    it('should return NotFound state when user lookup fails with EntityNotFoundException', async () => {
      ...
      urlGeneratorService.generateUrlForPlatform.mockReturnValue('https://example.com');

      const result = await service.resolveUrl('https://example.com/user/unknown', agentInfo);
    ...
    it('should return Forbidden state when authorization fails', async () => {
      ...
      const result = await service.resolveUrl('https://example.com/my-space', agentInfo);
    ...
    it('should resolve organization URL', async () => {
      ...
      organizationLookupService.getOrganizationByNameIdOrFail.mockResolvedValue(org);

      const result = await service.resolveUrl('https://example.com/organization/acme', agentInfo);
    ...
    it('should resolve SPACE_EXPLORER type', async () => {
      const result = await service.resolveUrl('https://example.com/spaces', agentInfo);

      expect(result.type).toBe(UrlType.SPACE_EXPLORER);
    });

    it('should resolve CONTRIBUTORS_EXPLORER type', async () => {
      const result = await service.resolveUrl('https://example.com/contributors', agentInfo);

      expect(result.type).toBe(UrlType.CONTRIBUTORS_EXPLORER);
    });

    it('should resolve FORUM type', async () => {
      const result = await service.resolveUrl('https://example.com/forum', agentInfo);

      expect(result.type).toBe(UrlType.FORUM);
    });

    it('should resolve INNOVATION_LIBRARY type', async () => {
      const result = await service.resolveUrl('https://example.com/innovation-library', agentInfo);

      expect(result.type).toBe(UrlType.INNOVATION_LIBRARY);
    });

    it('should resolve identity routes (login, logout, registration)', async () => {
      const loginResult = await service.resolveUrl('https://example.com/login', agentInfo);
      expect(loginResult.type).toBe(UrlType.LOGIN);

      const logoutResult = await service.resolveUrl('https://example.com/logout', agentInfo);
      expect(logoutResult.type).toBe(UrlType.LOGOUT);

      const registrationResult = await service.resolveUrl('https://example.com/registration', agentInfo);
