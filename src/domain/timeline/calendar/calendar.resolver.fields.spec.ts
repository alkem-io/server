...
describe('CalendarResolverFields', () => {
  ...
  let _spaceSettingsService: Mocked<SpaceSettingsService>;
  ...
  beforeEach(async () => {
    ...
    _spaceSettingsService = module.get(
  ...
  describe('events', () => {
    it('should return events with subspace bubbling when space settings allow it', async () => {
      ...
      calendarService.getSpaceFromCalendarOrFail.mockResolvedValue(
        mockSpace as unknown as Awaited<
          ReturnType<CalendarService['getSpaceFromCalendarOrFail']>
        >
      );
