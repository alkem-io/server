...
@Injectable()
export class CredentialsAuthGuard extends AuthGuard([
  'azure-ad',
  'demo-auth-jwt',
]) {
  ...
  canActivate(context: ExecutionContext) {
    ...
    const auth_credentials = this.reflector.get<string[]>(
      'credentials',
      context.getHandler()
    );
  ...
  matchcredentials(userInfo: UserInfo): boolean {
    ...
    return groups.some(
      ({ name }) =>
        name === AuthorizationRoles.GlobalAdmins ||
        this.credentials.includes(name)
    );
  }
  ...
  handleRequest(
    ...
    if (this.matchcredentials(userInfo)) return userInfo;
    throw new ForbiddenException(
