...
import { Challenge, Context, Reference } from '../models';
...
export async function populate_sample_challenge2(connection: Connection): Promise<void> {
    const challengeName = 'Energy Web2';

    // Get the ecoverse
    let challenge = await connection
        .createQueryBuilder()
        .select('challenge')
